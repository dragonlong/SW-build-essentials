...
while (1):
	...
	for steer in range(num):
		steeringAngle = 3.14
		p.setJointMotorControl2(dobot, steer, p.POSITION_CONTROL, targetVelocity=0, targetPosition=steeringAngle)
		time.sleep(0.01)
	for steer in range(num):
		lstate = p.getLinkState(dobot, linkIndex=steer, computeForwardKinematics=True)
		print(lstate)
	#print(eul)
